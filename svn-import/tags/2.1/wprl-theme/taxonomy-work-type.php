			...
			<?php if ( have_posts() ){ ?>
				...
				<header class="archive-header">
					...
					<h1 class="archive-title">
						...
						<?php _e('Archive for', 'wp_reading_list');  _e(' '.$value->name);?>
						...
						<?php if ($wprl_options['list_image']) 

								{
...

									{
										...
										_e('<a id="post-thumbnail" href="'.esc_url($worklink).'" target="_blank"/>');
									...
									if (has_post_thumbnail()) {
										...
										_e('<img src="'.esc_url($wprl_options['cover_image']).'" width="200" height="267">');

...
function wprl_custom_messages( $messages ) {
	 ...
	 $messages['works'] = array(
			...
			1 => sprintf( _e('Work updated. <a href="%s">View Work</a>', 'wp_reading_list'), esc_url( get_permalink($post_ID) ) ),
			2 => _e('Custom field updated.', 'wp_reading_list'),
			3 => _e('Custom field deleted.', 'wp_reading_list'),
			4 => _e('Work updated.', 'wp_reading_list'),
			5 => isset($_GET['revision']) ? sprintf( _e('Work restored to revision from %s', 'wp_reading_list'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => sprintf( _e('Work published. <a href="%s">View Work</a>', 'wp_reading_list'), esc_url( get_permalink($post_ID) ) ),
			7 => _e('Work saved.', 'wp_reading_list'),
			8 => sprintf( _e('Work submitted. <a target="_blank" href="%s">Preview Work</a>', 'wp_reading_list'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
			9 => sprintf( _e('Scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Work</a>', 'wp_reading_list'),
			date_i18n( _e( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
			10 => sprintf( _e('Draft updated. <a target="_blank" href="%s">Preview Work</a>', 'wp_reading_list'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
	  );
	return $messages;
}
